...
int gfx_syscall_hook(int option, void* data, int flags)
{
    ...
    ERR_ON_NULL(data);
    ERR_ON_NULL($current_process);

    switch (option){
    case GFX_DRAW_CHAR_OPT:;
        struct gfx_char* c = (struct gfx_char*)data;
        kernel_gfx_draw_char($current_process->gfx_window, c->x, c->y, c->data, c->color);
        break;
    case GFX_DRAW_RECTANGLE_OPT:;
        struct gfx_rectangle* r = (struct gfx_rectangle*)data;
        kernel_gfx_draw_rectangle($current_process->gfx_window, r->x, r->y, r->width, r->height, r->palette == GFX_VGA ? r->color : rgb_to_vga(r->color));
        break;
    
    case GFX_DRAW_CIRCLE_OPT:;
        struct gfx_circle* circle = (struct gfx_circle*)data;
        kernel_gfx_draw_circle($current_process->gfx_window, circle->x, circle->y, circle->r, circle->color, circle->fill);
        break;
    ...
    case GFX_DRAW_PIXEL:;
        ...
        kernel_gfx_draw_pixel($current_process->gfx_window, pixel->x, pixel->y, rgb_to_vga(pixel->color));
        break;
