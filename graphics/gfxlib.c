...
{
	return $current_process->gfx_window->inner_width;
}

int gfx_get_window_height()
{
	return $current_process->gfx_window->inner_height;
}
...
{
	kernel_gfx_draw_rectangle($current_process->gfx_window, x, y, width, height, color);
	kernel_gfx_draw_rectangle($current_process->gfx_window, x, y, width-1, 1, 31);
	kernel_gfx_draw_rectangle($current_process->gfx_window, x, y, 1, height, 31);
	kernel_gfx_draw_rectangle($current_process->gfx_window, x+width-1, y, 1, height, COLOR_VGA_MEDIUM_DARK_GRAY+5);
	kernel_gfx_draw_rectangle($current_process->gfx_window, x, y+height-1, width-1, 1, COLOR_VGA_MEDIUM_DARK_GRAY+5);
	kernel_gfx_draw_rectangle($current_process->gfx_window, x, y+height, width-1, 1, 31);
	...
	kernel_gfx_draw_text($current_process->gfx_window, text_x, text_y, name, 0x0);
	...
	for (int l = 0; l < 16; l++) {
		...
		for (int i = 0; i < 16; i++) {
			...
			if (icon[l*16+i] != 0xfa) {
				...
				if((x)+i < 0 || (y)+l < 0 || (x)+i > $current_process->gfx_window->inner_width || (y)+l > $current_process->gfx_window->inner_height)
					continue;
				putpixel($current_process->gfx_window->inner, (x)+i, (y)+l, rgb_to_vga(icon[l*16+i]), $current_process->gfx_window->pitch);
			}
		}
	}
	return 0;
	...
	for (int l = 0; l < 32; l++) {
		...
		for (int i = 0; i < 32; i++) {
			...
			if (icon[l*32+i] != 0xfa) {
				...
				if((x)+i < 0 || (y)+l < 0 || (x)+i > $current_process->gfx_window->inner_width || (y)+l > $current_process->gfx_window->inner_height)
					continue;
				putpixel($current_process->gfx_window->inner, (x)+i, (y)+l, rgb_to_vga(icon[l*32+i]), $current_process->gfx_window->pitch);
			}
		}
	}
	return 0;
...
int gfx_event_loop(struct gfx_event* event, gfx_event_flag_t flags)
{
	...
	ERR_ON_NULL($current_process->gfx_window);
	/**
	 ...
	 */
	while(1){
		if($current_process->gfx_window->events.tail == $current_process->gfx_window->events.head){
		...
		SPINLOCK($current_process->gfx_window, {
			memcpy(event, &$current_process->gfx_window->events.list[$current_process->gfx_window->events.tail], sizeof(struct gfx_event));
			$current_process->gfx_window->events.tail = ($current_process->gfx_window->events.tail + 1) % GFX_MAX_EVENTS;
		});
		return 0;
	}
}
...
void gfx_commit()
{
	if($current_process->gfx_window == NULL)
		return;
	$current_process->gfx_window->changed = 1;
}
...
int kernel_gfx_set_title(char* title)
{
	...
	memset($current_process->gfx_window->name, 0, GFX_MAX_WINDOW_NAME_SIZE);
	memcpy($current_process->gfx_window->name, title, strlen(title));
...
int kernel_gfx_set_header(const char* header)
{
	...
	memset($current_process->gfx_window->header, 0, GFX_MAX_WINDOW_NAME_SIZE);
	memcpy($current_process->gfx_window->header, header, strlen(header));
