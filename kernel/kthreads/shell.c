...
void shell_clear()
{
	...
	kernel_gfx_draw_rectangle($current_process->gfx_window, 0, SHELL_POSITION, gfx_get_window_height(), 8, theme->terminal.background);
}
...
void reset_shell()
{
	...
	kernel_gfx_draw_text($current_process->gfx_window, 0, SHELL_POSITION, shell_name, COLOR_VGA_MISC);
	shell_column += 1;
}
...
void ps(int argc, char* argv[])
{
	...
	if(spin){
		while(1){
			$current_process->term->ops->reset($current_process->term);
...
{
	kernel_gfx_draw_rectangle($current_process->gfx_window, 0,0, gfx_get_window_width(), gfx_get_window_height(), COLOR_VGA_BG);
	$current_process->term->ops->reset($current_process->term);
	reset_shell();
}
...
void shell_put(unsigned char c)
{
	...
	if(uc == backspace)
	{
		if(shell_buffer_length < 1)
			return;
		shell_column -= 1;
		kernel_gfx_draw_rectangle($current_process->gfx_window, shell_column*8, SHELL_POSITION, 8, 8, COLOR_VGA_BG);
		gfx_commit();
	...
	kernel_gfx_draw_char($current_process->gfx_window, shell_column*8, SHELL_POSITION, uc, COLOR_VGA_FG);
	gfx_commit();
