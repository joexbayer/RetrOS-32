...
int pcb_cleanup_routine(int pid)
{
	...
	dbgprintf("[PCB] Cleanup on PID %d (%s) stack: 0x%x (kstack: 0x%x)\n", pid, pcb_table[pid].name, pcb_table[pid].esp, pcb_table[pid].stack_ptr);
...
int pcb_init_kthread(int pid, struct pcb* pcb, void (*entry)(), char* name)
{
	...
	dbgprintf("Created new kernel thread!\n");
...
int pcb_create_process(char* program, int args, char** argv)
{
	...
	pcb->pid = i;
	pcb->eip = (void (*)()) VMEM_DATA; /* External programs start */
	pcb->esp = VMEM_STACK;
	pcb->data_size = read;
	pcb->ebp = pcb->esp;
...
{   
	...
	dbgprintf("All process control blocks are ready.\n");
